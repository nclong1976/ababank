...
import { Gift, X, Fingerprint } from 'lucide-react';
...
export default function Payment({
  ...
  return (
    <motion.div
      ...
      className="flex flex-col h-full bg-gradient-to-br from-[#005f73] to-[#0a9396] overflow-hidden"
    >
      <StatusBar />
          ...
          <div className="bg-white rounded-2xl p-6 mx-6 max-w-sm w-full">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-3.5 bg-gray-100 rounded-xl font-bold text-gray-600 text-sm active:bg-gray-200"
              >
                Cancel
              </button>
              <button
                onClick={handleBiometricAuth}
                className="flex-1 py-3.5 bg-[#37a7b8] rounded-xl font-bold text-white text-sm"
              >
      ...
      <div className="flex-1 flex flex-col px-4 pb-2 overflow-hidden">
        {step === 'input' ? (
          <>
            <div className="flex flex-col items-center mb-2">
              ...
              <h2 className="text-white font-bold text-sm text-center max-w-[240px] truncate">
                {recipientName}
              </h2>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-white text-sm font-bold active:bg-white/30"
              >
                {currency}
              </button>
            </div>
            ...
            <div className="mb-2">
              <div className="text-white/70 text-xs font-medium mb-1 text-center">
                Note:
              </div>
              <div className="flex gap-1.5 overflow-x-auto no-scrollbar justify-center">
                <button
                  onClick={() => setIsGift(!isGift)}
                  className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full border transition-all font-bold text-xs ${
                    isGift
                ...
                {remarks
                  ...
                  .map((r) => (
                    <button
                      key={r}
                      onClick={() => setRemark(r)}
                      className={`flex-shrink-0 px-4 py-1.5 rounded-full border transition-all font-bold text-xs ${
                        remark === r
          ...
          <div className="flex flex-col items-center justify-center flex-1">
              ...
              {[...Array(4)].map((_, i) => (
                <div
                  ...
                  className={`w-3 h-3 rounded-full border-2 transition-all ${
                    i < pin.length
                      ? 'bg-[#37a7b8] border-[#37a7b8] scale-125'
            ...
            {isProcessing ? (
              ...
              <button className="text-white/70 text-xs mt-2">Forgot PIN?</button>
            )}
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2 mb-2">
          {[
            ...
            <button
              ...
              className="h-11 flex items-center justify-center text-white text-2xl font-medium active:bg-white/5 rounded-full transition-colors disabled:opacity-30"
        ...
        {step === 'input' && (
          <button
            ...
            className="w-full bg-[#ff5252] text-white font-bold py-3 rounded-full active:bg-[#e04848] transition-colors disabled:opacity-40 safe-bottom"
          >
            NEXT
          </button>
        )}
